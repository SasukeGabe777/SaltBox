/**
 * local-service-clean, version 2.0.0 — the typography-led composition,
 * rendering demo-content-v3 as a customer-facing site (service rows with
 * calls to action and optional photos, why-us points, how-it-works process,
 * about photo; one footer disclosure instead of in-page demo badges).
 *
...
export function renderLocalServiceCleanV2(content: DemoContent, options: RenderOptions = {}): string {
...
${baseCss()}
${blocksCssV2()}
.service-row .thumb{height:170px;border-radius:12px;margin-top:16px}
...
<section class="hero" id="top" data-section="hero" data-improve-anchor="hero-section">
  <div class="container">
    ...
    <h1 data-improve-anchor="hero">${esc(content.hero.headline)}</h1>
...
<section id="services" data-section="services">
  <div class="container">
    ...
    <div class="service-rows" data-improve-anchor="services">
      ${content.services.items
        .map(
          (item, index) => `<article class="service-row">
        <span class="num" aria-hidden="true">${String(index + 1).padStart(2, "0")}</span>
        <div><h3>${esc(item.title)}</h3></div>
        <div><p>${esc(item.description)}</p>${serviceLink(item)}${serviceThumb(item)}</div>
      </article>`,
        )
        .join("\n      ")}
    </div>
...
${
  ...
  <div class="container">
    ...
    <h2>${esc(content.process.heading)}</h2>

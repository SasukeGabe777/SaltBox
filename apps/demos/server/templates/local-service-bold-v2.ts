/**
 * local-service-bold, version 2.0.0 — the high-contrast trade composition,
 * rendering demo-content-v3 as a customer-facing site (per-service photos
 * and calls to action, why-us band, how-it-works process, about photo; one
 * footer disclosure instead of in-page demo badges).
 *
...
export function renderLocalServiceBoldV2(content: DemoContent, options: RenderOptions = {}): string {
...
${baseCss()}
${blocksCssV2()}
.service-card .thumb{height:180px;margin:-4px -4px 18px;border-radius:10px}
.service-card{display:flex;flex-direction:column}
.service-card .service-link{margin-top:auto;padding-top:12px}
  ...
  <div class="container">
    ...
    <div class="services-grid" data-improve-anchor="services">
      ${content.services.items
        ...
        ${serviceThumb(item)}
        <h3>${item.image ? "" : ICONS.check}${esc(item.title)}</h3>
  ...
  <div class="container">
    ...
    <div class="contact-grid">
      <div class="form-card">
        ...
        ${quoteFormV2(content, "contact-qf", "contact-form")}
      </div>
      <aside>

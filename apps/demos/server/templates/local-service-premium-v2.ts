/**
 * local-service-premium, version 2.0.0 — the image-forward composition,
 * rendering demo-content-v3 as a customer-facing site: service cards with
 * the business's own per-service photos and calls to action, why-us points,
 * a how-it-works process, and an about photo. No demo badges or notes in
 * the page body; the footer carries the single preview disclosure.
 *
...
export function renderLocalServicePremiumV2(content: DemoContent, options: RenderOptions = {}): string {
...
.service-card .thumb{margin:-30px -26px 22px;height:200px}
.service-card .service-link{margin-top:auto;padding-top:14px}
...
<header class="site-header">
  <div class="container header-row">
    ...
    <nav class="site-nav" aria-label="Main navigation">
      ...
      <a class="btn btn-accent btn-sm" href="${esc(ctaHref(content.hero.primaryCta.kind, content))}" data-qa="header-cta" data-improve-anchor="header-cta">${esc(content.hero.primaryCta.label)}</a>
    </nav>
  </div>
</header>
...
<section class="hero" id="top" data-section="hero" data-improve-anchor="hero-section">
  ...
  <div class="container hero-content">
    ...
    <div class="cta-row">
      <a class="btn btn-accent" href="${esc(ctaHref(content.hero.primaryCta.kind, content))}" data-qa="primary-cta" data-improve-also="quote">${esc(content.hero.primaryCta.label)}</a>
    ...
    <div data-improve-anchor="hero-contact">${heroFactChips(content)}</div>
  </div>
</section>
...
<section id="services" data-section="services">
  <div class="container">
    ...
    <div class="services-grid" data-improve-anchor="services">
      ${content.services.items
        .map(
          (item) => `<article class="service-card">
        ${serviceThumb(item)}
        ${item.image ? "" : `<div class="icon">${ICONS.check}</div>`}
...
${
  ...
  <div class="container">
    ...
    <h2 class="display">Project Gallery</h2>
...
${
  content.process
    ? `<section class="process" data-section="process">
  <div class="container">
    <p class="section-kicker">How it works</p>
    <h2 class="display">${esc(content.process.heading)}</h2>
...
${aboutSectionV2(content, "display")}
  ...
  <div class="container">
    ...
    <div class="contact-grid">
      <div class="form-card">
        ...
        ${quoteFormV2(content, "qf", "contact-form")}
      </div>
...
<div data-improve-anchor="footer">${footerBlock(content)}</div>
${demoIndicator(content)}
${inlineScriptV2()}
${improvementsLayer(content, options)}
</body>
</html>
`;
}
